(function() {
        ...
        <div class="inj-body">
            ...
            <div id="inj-view-dapodik" style="display:none;">
                ...
                <div style="border-top:1px solid #eee; padding-top:15px;">
                    ...
                    <button type="button" class="inj-action-btn" id="btn-tambah-bangunan">🏗️ Tambah Bangunan</button>
                    <button type="button" class="inj-action-btn" id="btn-pindah-rombel">🔄 Pindah Rombel</button>
    ...
    // FUNGSI SIMULASI KLIK BOOKMARK ASLI (ANTI-ERROR UNTUK SCRIPT TERENKRIPSI)
    ...
    // Tombol Tambah Bangunan (BARU)
    document.getElementById('btn-tambah-bangunan').onclick = function(e) {
        e.preventDefault(); e.stopPropagation();
        const password = prompt("Masukkan password untuk menjalankan script:");
        if (password === "IBNU123") {
            try {
                const targetEl = document.getElementById('toolbar-1131-targetEl');
                if (targetEl) targetEl.style.width = '926px';

                const separator = document.getElementById('tbseparator-1133');
                if (separator) separator.style.left = '41px';

                const btnTambah = document.getElementById('button-1134');
                if (btnTambah) { btnTambah.style.display = ''; btnTambah.style.left = '440px'; }

                const btnUbah = document.getElementById('button-1135');
                if (btnUbah) { btnUbah.style.display = ''; btnUbah.style.left = '355px'; }

                const btnSimpan = document.getElementById('button-1136');
                if (btnSimpan) { btnSimpan.style.display = ''; btnSimpan.style.left = '855px'; }

                const btnHapus = document.getElementById('button-1137');
                if (btnHapus) { btnHapus.style.display = ''; btnHapus.style.left = '766px'; }

                const btnValidasi = document.getElementById('button-1138');
                if (btnValidasi) btnValidasi.style.left = '51px';

                const btnBlockgrant = document.getElementById('button-1163');
                if (btnBlockgrant) btnBlockgrant.style.left = '152px';

                const btnHapusBuku = document.getElementById('button-1164');
                if (btnHapusBuku) { btnHapusBuku.style.display = ''; btnHapusBuku.style.left = '540px'; }

                alert("Password benar! Tombol-tombol berhasil dimunculkan.");
            } catch(err) {
                alert("Terjadi kesalahan atau Anda tidak berada di halaman yang tepat.");
            }
        } else {
            if(password !== null) alert("Password salah atau dibatalkan! Script tidak dijalankan.");
        }
    };
    ...
    document.getElementById('btn-input-pembelajaran').onclick = function(e) {
        ...
        jalankanBookmarkletAsli(`/* PASTE KODE INPUT PEMBELAJARAN ASLI ANDA DI SINI */`);
    };
